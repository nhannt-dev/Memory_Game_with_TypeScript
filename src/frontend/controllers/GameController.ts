...
export class GameController {
    ...
    renderHTML(rootElement: HTMLElement, item: GameItem){
        ...
        //     <img src="https://cdn.pixabay.com/photo/2019/11/27/14/06/pokemon-4657023_960_720.png" alt="" class="img-fluid">
        // </div>
    ...
    isMatched(id: number, imgElement: HTMLImageElement): boolean {
        ...
        if(openedItems.length == 2){
            let checkMarkFilter = openedItems.filter(item=>item.id == id)
            
            if(checkMarkFilter.length < 2){
    ...
    changeMatchedBackground(imgElement: HTMLElement | null, isMatched: boolean = true) {
        if(imgElement?.parentElement){
            if(isMatched){
                imgElement.parentElement.className = 'col-2 gameItem m-1 p1 text-center'
            }else {
                imgElement.parentElement.className = 
                'col-2 gameItem m-1 p1 text-center unmatched'
            }
        }
    }
    @autobind
